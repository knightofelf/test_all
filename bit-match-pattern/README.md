...
[사용자 입력값 N]
 ...
 └───────────────────────────────┘
        │
        ▼
 [3대 파라미터: depthA, depthB, depthBPos]
        │
        ▼
 ┌───────────────────────┐
 │   fn_BitDepthPos_decode│
 └───────────────────────┘
        │
        ▼
 ┌───────────────────────────────┐
 │ startNode = 2^(depthA-1)       │
 │ restoredOrder = depthBPos <<   │
 │                (depthA-depthB) │
 │ 오차 보정(globalSavedInput 활용)│
 └───────────────────────────────┘
        │
        ▼
 [복원된 정수 = 원래 입력값 N]
 ...
 [저장 버튼]         [불러오기 버튼]
   │                     │
   ▼                     ▼
 localStorage ← 저장   localStorage → 불러오기
   │                     │
   ▼                     ▼
 [depthA, depthB, depthBPos, savedInput]
   │                     │
   ▼                     ▼
 다시 디코딩 → 원래 입력값 복원
...
//-----------------------------------------------------
✅ 깊이A + (깊이B. 깊이B의 순서위치(0보다 큰) = 깊이A의 한줄 순서를 복원) = 숫자 복원
//-----------------------------------------------------

숫자 = 비트 : 54345345 = 11001111010011111010000001₂
전체 숫자 순서 : 54345345
숫자 재귀순서 깊이A : 26
숫자 재귀순서 깊이A의 한줄 순서 : 20790914
숫자 재귀순서 깊이A의 한줄 순서/2 깊이A전까지 반복 나누기. 깊이B : -24
숫자 재귀순서 깊이A의 한줄 순서/2 깊이A전까지 반복 나누기. 깊이B의 순서 위치값 : 1
깊이B와 깊이B의 순서위치로 복원된. 깊이A값과 깊이A의 한줄 순서값 : 깊이A: 26, 한줄 순서값: 20790914
깊이B와 깊이B의 순서위치로 복원된. 숫자 복원값 : 54345345
깊이 A 와 깊이B . 깊이B의 순서위치'로. 숫자 복원이 되는거지? o x

(예제형식) 15 깊이A4 순서 8번째
깊이A1 1 (1)
깊이A2 10 (0)11 (1)  깊이B -2 깊이순서B 2
깊이A3 100 (00)101 (01)110 (0)111 (1)
깊이A4 1000 (000)1001 (001)1010 (010)1011 (011)1100 (00)1101 (01)1110 (0)1111 (1)   순서A 8번째
...
깊이B와 깊이B의 순서위치로 복원된. 깊이A값과 깊이A의 한줄 순서값 : 깊이A: 26, 한줄 순서값: 20790914
깊이B와 깊이B의 순서위치로 복원된. 숫자 복원값 : 54345345
깊이 A 와 깊이B . 깊이B의 순서위치면. 숫자 복원이 되는거지? o x
...
//-----------------------------------------------------
    🌟핵심 : 비트 위치 = 범위 찾기 (껍질 비트 or 공통 비트) = N방향 깊이 찾기 or 벡터 = 앞 공통비트 제거. 남은 비트 자릿수 재귀수열
...
//-----------------------------------------------------
     🔎 재귀적 구조 (비트 재귀수열)     :  껍질용 나머지 - 구조 접근성 이해에 도움
f(n) = {prefix}(n) + f(n - 2^{k})

- ({prefix}(n)) = 가장 앞자리 공통 비트  
- (k) = 이진수 자릿수 - 1  
- (f(n - 2^k)) = 앞자리 제거 후 남은 부분 → 이전 껍질 패턴과 동일 
...
:: Step 1. 입력 값 설정
set n=3823957928339845723 
11010100001001011101001001001011101001001001001001011   52비트
11010100       8비트를 제외한. 남은 부분
212                3311410329608795  안씀…


:: Step 2. prefix 선택 (앞 8비트 = 11010100₂ = 212)
set prefix=212
2^53 =  52비트+1 


:: Step 3. 껍질 크기 계산 (212 × 2^53)                2^53 = 9007199254740992
set shell=212 * 9007199254740992                   212 * 9007199254740992 = 1910323259005118464 
:: shell = 1910323259005118464                        


:: Step 4. 내부 알맹이 계산
set inner=%n% - %shell%                                   3823957928339845723 - 1910323259005118464  = 1913634669334727259
:: inner = 1913634669334727259


:: Step 5. 최종 공식 출력
echo f(%n%) = 11010100₂ + f(%inner%)             f(3823957928339845723 ) = 11010100₂ + f(1913634669334727259 )
...
✅비트 - 껍질 복원 공식
//-----------------------------------------------------
입력 = 3823957928339845723


//-----------------------------------------------------
속껍질
// f( n ) = 11010100₂ + f(1913634669334727259)   


:: Step 1. prefix 선택
set prefix=11010100   :: 212


:: Step 2. 껍질 크기 계산
set shell=212 * 2^53                                2^53 = 9007199254740992        212 * 9007199254740992 = 1910323259005118464 
:: shell = 1910323259005118464


:: Step 3. 내부 알맹이 계산
set inner=1913634669334727259 - 1910323259005118464
:: inner = 3311410329608795


:: Step 4. 공식 출력
echo f(1913634669334727259) = 11010100₂ + f(3311410329608795)
//-----------------------------------------------------
...
// 복구
f(3823957928339845723) = 11010100_{2} + ((11010100_{2} + f(3311410329608795))
//-----------------------------------------------------
