...
// 부메랑 발사 및 BigInt 우주 역학 계산
function fireBoomerang() {
    if (isFlying) return;

    const degInput = parseInt(document.getElementById('angleInput').value, 10);
    const vRawStr = parseToExactIntegerString(document.getElementById('velocityInput').value);

    if (!vRawStr || isNaN(degInput) || degInput < 0 || degInput > 90) {
        alert("정확한 각도(0~90)와 초기 속도를 입력하세요!");
        return;
    }

    const vBig = BigInt(vRawStr);
    const realDeg = (turn === 1) ? degInput : 180 - degInput;
    const rad = realDeg * (Math.PI / 180);

    // ---------------- [ BigInt 우주 물리 엔진 연산 파트 ] ----------------
    const sinVal = BigInt(Math.round(Math.sin(realDeg * Math.PI / 180) * Number(SCALE)));
    const sin2Theta = BigInt(Math.round(Math.sin(2 * realDeg * Math.PI / 180) * Number(SCALE)));

    // 1. 총 비행 시간 계산 (밀리초)
    const totalTimeMs = (2n * vBig * BigInt(Math.abs(Number(sinVal))) * 1000n) / (gBig * SCALE);

    // 2. 최고 도달 높이 계산 (BigInt 단위 정수 출력)
    const vSinScaled = (vBig * BigInt(Math.abs(Number(sinVal)))) / SCALE;
    const maxHeightBig = (vSinScaled * vSinScaled) / (2n * gBig);
    ...
    document.getElementById('physicsLog').innerHTML = `
        🚀 <b>[우주 역학 데이터 보고서]</b><br>
        • 던진 주체: 플레이어 ${turn} 고릴라<br>
        • 수평 발사 속도 수치: ${vBig.toLocaleString()} m/s (각도: ${degInput}°)<br>
        • ⏱️ 계산된 총 체공 시간: <b>${totalTimeMs.toLocaleString()}</b> 밀리초<br>
        • 📐 <b>최고 도달 높이 (Y축 Peak):</b> <span style="color:#e74c3c; font-weight:bold;">${maxHeightBig.toString()} m</span><br>
        • 🎯 예상 수평 도달 거리: <b>${totalDistanceBig.toString()} m</b>
    `;
    // ---------------------------------------------------------------------
